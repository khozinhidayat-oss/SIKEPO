...
import { 
  UserRole, DisciplineStatus, StudentPointSummary, Transaction, ClassItem, MajorItem, SystemSettings
} from '../types';
import { 
...
import { 
  ...
  Layers, School, GraduationCap, ShieldAlert, AlertTriangle, FileText, Loader2, Gauge
} from 'lucide-react';
...
export const ReportsView: React.FC<ReportsViewProps> = ({ role, userName }) => {
  ...
  const [majors, setMajors] = useState<MajorItem[]>(getMajors());
  ...
  const [selectedPointRange, setSelectedPointRange] = useState<string>('ALL');
  ...
  // Fetch live data directly from REST API / Spreadsheet Backend
  const fetchReportData = useCallback(async () => {
    setIsDataLoading(true);
    try {
      const [stdRes, trxRes, clsRes, mjrRes, setRes] = await Promise.all([
      ...
      const stdData = stdRes.success && Array.isArray(stdRes.data) ? stdRes.data : getStudents();
      const trxData = trxRes.success && Array.isArray(trxRes.data) ? trxRes.data : getTransactions();
      const clsData = clsRes.success && Array.isArray(clsRes.data) ? clsRes.data : getClasses();
      const mjrData = mjrRes.success && Array.isArray(mjrRes.data) ? mjrRes.data : getMajors();
      const setData = setRes.success && setRes.data ? setRes.data : getSettings();
      ...
      const computedSummaries = getStudentPointSummaries(stdData, trxData, setData);
      setStudents(computedSummaries);
    } catch (err) {
  ...
  const availableClasses = useMemo(() => {
    ...
    students.forEach(s => {
      const sLevel = s.level || getStudentLevelInfo(s.className, classes).code;
  ...
  // Multi-Filtered Students Calculation
  const filteredStudents = useMemo(() => {
    return students.filter(s => {
      ...
      const levelCode = s.level || getStudentLevelInfo(s.className, classes).code;
      ...
      const matchesMajor = 
        selectedMajor === 'ALL' || 
        (s.majorName && s.majorName.toLowerCase() === selectedMajor.toLowerCase()) ||
        (s.className && s.className.toLowerCase().includes(selectedMajor.toLowerCase()));

      const matchesTingkat = 
        selectedTingkat === 'ALL' || 
        levelCode === selectedTingkat;
      ...
      const matchesPointRange = (() => {
        if (selectedPointRange === 'ALL') return true;
        const pts = Number(s.totalPelanggaran || 0);
        if (selectedPointRange === '0') return pts === 0;
        if (selectedPointRange === '>0') return pts > 0;
        if (selectedPointRange === '1-19') return pts >= 1 && pts <= 19;
        if (selectedPointRange === '20-39') return pts >= 20 && pts <= 39;
        if (selectedPointRange === '40-59') return pts >= 40 && pts <= 59;
        if (selectedPointRange === '60-79') return pts >= 60 && pts <= 79;
        if (selectedPointRange === '80-99') return pts >= 80 && pts <= 99;
        if (selectedPointRange === '>=100') return pts >= 100;
        return true;
      })();

      return matchesSearch && matchesMajor && matchesTingkat && matchesClass && matchesStatus && matchesPointRange;
    }).sort((a, b) => {
    ...
    selectedTingkat, selectedClass, selectedStatusFilter, selectedPointRange, sortField, sortDirection, classes
  ]);
  ...
  const handleExportExcel = () => {
    const exportData = filteredStudents.map((s, idx) => {
      ...
      return {
        ...
        Jurusan: s.majorName,
        Tingkat: s.level || levelInfo.label,
        Kelas: s.className,
        JumlahPelanggaran: s.totalViolationsCount || 0,
        ...
        TanggalPelanggaranTerakhir: s.lastViolationDate || '-'
      };
    });
  ...
  const studentViolations = useMemo(() => {
    ...
    const sId = String(selectedDetailStudent.id || '').trim().toLowerCase();
    const sNis = String(selectedDetailStudent.nis || '').trim().toLowerCase();
    const sName = String(selectedDetailStudent.name || '').trim().toLowerCase();
    ...
    }).sort((a, b) => new Date(b.date || (b as any).tanggal || 0).getTime() - new Date(a.date || (a as any).tanggal || 0).getTime());
  }, [selectedDetailStudent, transactions]);

  const latestUpdatedDate = useMemo(() => {
    if (studentViolations.length === 0) return selectedDetailStudent?.lastViolationDate || '-';
    const top = studentViolations[0];
    return top.date + (top.time ? ` ${top.time}` : '');
  }, [studentViolations, selectedDetailStudent]);
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-slate-800 p-6 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xs print:hidden">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Penetapan <strong>Status Kedisiplinan</strong>, <strong>Jenis Peringatan</strong>, dan <strong>Tindak Lanjut</strong> secara otomatis berdasarkan total poin akumulasi transaksi pelanggaran.
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <RefreshCw className={`w-4 h-4 text-blue-600 ${isDataLoading ? 'animate-spin' : ''}`} />
            <span className="hidden sm:inline">{isDataLoading ? 'Mengambil Data...' : 'Refresh Data'}</span>
          </button>
      ...
      <div className="bg-white dark:bg-slate-800 p-5 rounded-3xl border border-slate-100 dark:border-slate-700 shadow-xs space-y-4 print:hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-slate-700 pb-3">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-800 dark:text-slate-200">
            ...
            <span>Multi-Filter Laporan Kedisiplinan Siswa:</span>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 text-xs">
          ...
          {/* Rentang Point Filter */}
          <div>
            <label className="block font-bold text-slate-600 dark:text-slate-400 mb-1 flex items-center gap-1">
              <Gauge className="w-3.5 h-3.5 text-red-500" />
              <span>Rentang Point:</span>
            </label>
            <select
              value={selectedPointRange}
              onChange={e => {
                setSelectedPointRange(e.target.value);
                setCurrentPage(1);
              }}
              className="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold dark:text-white"
            >
              <option value="ALL">Semua Point</option>
              <option value="0">0 Poin (Belum ada Pelanggaran)</option>
              <option value=">0">Ada Pelanggaran (&gt; 0 Poin)</option>
              <option value="1-19">1 – 19 Poin</option>
              <option value="20-39">20 – 39 Poin</option>
              <option value="40-59">40 – 59 Poin</option>
              <option value="60-79">60 – 79 Poin</option>
              <option value="80-99">80 – 99 Poin</option>
          ...
          {/* Search Input */}
          <div>
            <label className="block font-bold text-slate-600 dark:text-slate-400 mb-1">Cari (Nama / NIS / NISN / Kelas):</label>
      ...
      <div className="bg-white dark:bg-slate-800 p-4 rounded-2xl border border-slate-100 dark:border-slate-700 shadow-xs flex items-center justify-between gap-4 print:hidden">
        <div className="text-xs text-slate-500 font-medium flex items-center gap-2">
          {isDataLoading && <Loader2 className="w-4 h-4 text-blue-600 animate-spin" />}
          <span>
                ...
                <tr>
                  <td colSpan={10} className="p-12 text-center text-slate-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      ...
                      <span>Memuat data laporan dari Google Spreadsheet...</span>
                    </div>
                  </td>
                </tr>
                  ...
                  return (
                    <tr key={s.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/40 transition-colors">
                      ...
                      <td className="p-4 font-mono font-bold text-slate-800 dark:text-slate-200">
                        {s.nis}
                        {s.nisn && <div className="text-[10px] text-slate-400 font-normal">NISN: {s.nisn}</div>}
                      </td>
                      ...
                      <td className="p-4 text-center print:hidden">
                        <button
                          ...
                          title="Lihat Detail Siswa & Riwayat Transaksi"
                        >
