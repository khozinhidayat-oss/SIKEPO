...
export const MasterViolationsView: React.FC<MasterViolationsViewProps> = ({ role, userName }) => {
  ...
  const fetchViolations = async () => {
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        ...
        setViolations(getMasterViolations());
      }
    } catch (e) {
  ...
  const sortedViolations = useMemo(() => {
    return [...filteredViolations].sort((a, b) => {
      ...
      if (sortField === 'points') {
        const pA = Number(a.points !== undefined ? a.points : ((a as any).point || 0));
        const pB = Number(b.points !== undefined ? b.points : ((b as any).point || 0));
  ...
  const handleSaveForm = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isSaving || !name.trim() || !category.trim()) return;

    setIsSaving(true);
    showLoading(editingId ? 'Mengupdate master pelanggaran...' : 'Menyimpan master pelanggaran...');
    try {
      const res = await ApiService.saveViolation({
      ...
      if (res.success) {
        logActivity(
          ...
          `Master: ${code || ''} - ${name} (${points} Poin)`
        );
        setIsFormOpen(false);
        await fetchViolations();
        showToast(res.message || `Data master pelanggaran "${name}" berhasil disimpan.`, 'success');
      } else {
        showToast(res.message || 'Gagal menyimpan master pelanggaran.', 'error');
      }
    } catch (err: any) {
  ...
  const handleDelete = (v: MasterViolation) => {
    ...
    setAlertState({
      ...
      message: `Apakah Anda yakin ingin menghapus master pelanggaran "${v.name}" (${v.code || '-'}) dari database Google Spreadsheet?`,
      showCancel: true,
      onConfirm: async () => {
        showLoading('Menghapus master pelanggaran...');
        try {
          const res = await ApiService.deleteViolation(v.id);
          if (res.success) {
            logActivity(userName, role, 'HAPUS_MASTER_PELANGGARAN', `Master: ${v.code || ''} - ${v.name}`);
            await fetchViolations();
            showToast(res.message || 'Master pelanggaran berhasil dihapus.', 'success');
          } else {
            showToast(res.message || 'Gagal menghapus master pelanggaran.', 'error');
          }
        } catch (err: any) {
          showToast(err.message || 'Gagal terhubung ke backend.', 'error');
        } finally {
  ...
  const validateImportJson = (jsonRows: any[]) => {
    const existingDbCodes = new Set(violations.map(v => (v.code || '').trim().toUpperCase()).filter(Boolean));
  ...
  const handleExecuteImport = async () => {
    ...
    showLoading('Mengimpor master pelanggaran ke Google Spreadsheet...', 25);
    try {
      const res: any = await ApiService.importViolations(validRowsToImport);
      ...
      if (res.success) {
        logActivity(
          ...
          `Import Master Pelanggaran (${res.inserted || 0} baru, ${res.updated || 0} diperbarui)`
        );
        ...
        setAlertState({
          ...
          message: `Proses import selesai!\n\n• Data Baru Diberikan: ${res.inserted || 0}\n• Data Diperbarui: ${res.updated || 0}\n• Data Gagal/Invalid: ${totalInvalidCount}`,
          onConfirm: () => setAlertState(prev => ({ ...prev, isOpen: false }))
        });
      } else {
        showToast(res.message || 'Gagal mengimpor master pelanggaran.', 'error');
      }
    } catch (err: any) {
      showToast(err.message || 'Gagal terhubung ke backend.', 'error');
    } finally {
              ...
              {isDataLoading ? (
                <tr>
                  <td colSpan={isAdmin ? 7 : 6} className="p-12 text-center text-slate-400">
                    <RefreshCw className="w-8 h-8 animate-spin mx-auto mb-2 text-red-500 opacity-80" />
                    <div className="font-semibold text-slate-600 dark:text-slate-300">Memuat data master pelanggaran dari Google Spreadsheet...</div>
                  </td>
                </tr>
