...
import { 
  ...
  restoreFullBackupData, resetAllPoints, logActivity, getDisciplineRules 
} from '../utils/storage';
...
import { 
  ...
  Clock, RefreshCw, CheckCircle2, ShieldX, UserCog, Layers
} from 'lucide-react';
import { SweetAlertModal, AlertType } from '../components/SweetAlertModal';
import { DisciplineMatrixManager } from '../components/DisciplineMatrixManager';
...
type TabType = 'profil' | 'tahun_ajaran' | 'poin' | 'matriks_kedisiplinan' | 'keamanan' | 'tampilan' | 'backup' | 'tentang';

export const SettingsView: React.FC<SettingsViewProps> = ({ role, userName }) => {
  const isAdmin = role === 'admin';
  const [activeTab, setActiveTab] = useState<TabType>(isAdmin ? 'profil' : 'matriks_kedisiplinan');
  ...
  const fetchSettingsData = async () => {
    try {
      ...
      if (res.success && res.data) {
        ...
        if (res.data.schoolLogoUrl) setLogoPreview(res.data.schoolLogoUrl);
  ...
  // Access Control: Allow Admin (Full) and Kesiswaan (Read-Only Matriks Kedisiplinan)
  if (!isAdmin && role !== 'kesiswaan') {
    return (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 max-w-lg w-full text-center shadow-2xl border border-rose-200 dark:border-rose-900/50 space-y-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">
              Halaman <strong>Setting Sistem & Konfigurasi Aplikasi</strong> bersifat rahasia dan hanya dapat diakses oleh akun berkewenangan <strong>Administrator</strong> atau <strong>Kesiswaan</strong>.
            </p>
          </div>
          <div className="pt-2">
            <button
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    ApiService.saveSettings(settings).then(res => {
      setIsSaving(false);
      if (res.success) {
        ...
        setAlertState({
          ...
          message: res.message || 'Seluruh parameter konfigurasi aplikasi berhasil diperbarui dan disinkronkan ke Google Spreadsheet.',
          onConfirm: () => setAlertState(prev => ({ ...prev, isOpen: false }))
        });
      } else {
        setAlertState({
          ...
          message: res.message || 'Gagal menyimpan pengaturan ke Google Apps Script.',
          onConfirm: () => setAlertState(prev => ({ ...prev, isOpen: false }))
        });
      }
    }).catch(err => {
  ...
  const allTabsConfig = [
    ...
    { id: 'matriks_kedisiplinan' as TabType, label: 'Matriks Kedisiplinan', icon: Layers },
  ...
  const tabsConfig = isAdmin 
    ? allTabsConfig 
    : allTabsConfig.filter(t => t.id === 'matriks_kedisiplinan' || t.id === 'profil' || t.id === 'tentang');
      ...
      <form onSubmit={handleSaveSettings} className="space-y-6">
        ...
        {activeTab === 'poin' && (
          <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-6 animate-in fade-in duration-200">
            ...
            {/* Discipline Rules Link Banner */}
            <div className="p-4 bg-indigo-50 dark:bg-indigo-950/40 rounded-2xl border border-indigo-200 dark:border-indigo-800 flex items-center justify-between gap-3 text-xs">
              <div className="flex items-center gap-3">
                <div className="p-2.5 bg-indigo-600 text-white rounded-xl font-bold">
                  <Layers className="w-5 h-5" />
                </div>
                <div>
                  <h4 className="font-extrabold text-indigo-950 dark:text-indigo-200">Matriks Aturan Status Kedisiplinan Sekolah</h4>
                  <p className="text-[11px] text-indigo-700 dark:text-indigo-300">
                    Aturan poin, tindakan sekolah, pemanggilan ortu, dan skorsing kini dikelola secara terpusat melalui Matriks Master.
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setActiveTab('matriks_kedisiplinan')}
                className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-xs shrink-0 cursor-pointer transition-all shadow-md"
              >
                Buka Master Matriks
              </button>
            </div>
        ...
        {/* TAB 4: MATRIKS ATURAN KEDISIPLINAN SEKOLAH (MASTER RULE ENGINE) */}
        {activeTab === 'matriks_kedisiplinan' && (
          <DisciplineMatrixManager
            userRole={role}
            userName={userName}
