...
import { 
  getStudents, getMasterViolations, addTransaction, deleteTransaction, 
...
export const TransactionPelanggaranView: React.FC<TransactionPelanggaranViewProps> = ({
  ...
  const [isViolationsLoading, setIsViolationsLoading] = useState<boolean>(false);
    ...
    try {
      ...
      if (res.success && Array.isArray(res.data)) {
        const mappedData = res.data
          .filter((t: any) => !t.type || t.type === 'pelanggaran')
          .map((t: any) => ({
            ...t,
            type: 'pelanggaran' as const,
            studentNis: t.studentNis || t.nis || '',
            studentName: t.studentName || t.nama_siswa || t.nama || '',
            className: t.className || t.studentClass || t.kelas || '',
            majorName: t.majorName || t.jurusan || '',
            level: t.level || t.tingkat || (t.className ? (t.className.startsWith('XII') ? 'XII' : t.className.startsWith('XI') ? 'XI' : 'X') : 'X'),
            itemId: t.itemId || t.violationId || t.id_pelanggaran || '',
            itemName: t.itemName || t.violationName || t.nama_pelanggaran || '',
            itemCategory: t.itemCategory || t.category || t.kategori || 'Kedisiplinan',
            points: Number(t.points !== undefined ? t.points : (t.poin !== undefined ? t.poin : 0)),
            officerName: t.officerName || t.petugas || t.pelapor || 'Admin',
            notes: t.notes || t.keterangan || t.catatan || '',
            date: t.date || t.tanggal || new Date().toISOString().split('T')[0],
            time: t.time || t.waktu || '07:00'
          }));
        setHistoryTransactions(mappedData);
      } else {
        setHistoryTransactions(getTransactions().filter(t => !t.type || t.type === 'pelanggaran'));
      }
    } catch (e) {
  ...
  const fetchMasterViolationsData = async () => {
    setIsViolationsLoading(true);
    try {
      const res = await ApiService.getViolations();
      if (res.success && Array.isArray(res.data)) {
        setMasterViolations(res.data.filter((v: any) => v.status === 'Aktif'));
      } else {
        setMasterViolations(getMasterViolations().filter(v => v.status === 'Aktif'));
      }
    } catch (e) {
      console.error('Error fetching master violations:', e);
      setMasterViolations(getMasterViolations().filter(v => v.status === 'Aktif'));
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Save transaction via REST API & Storage Engine
    setIsSaving(true);
    showLoading('Mencatat transaksi pelanggaran...');
    try {
      ...
      if (res.success) {
        ...
        const newNetPoints = (selectedStudent.netPoints || 0) + selectedViolation.points;
        await fetchTransactionsData();
        handleResetForm();
        showToast(res.message || `Pelanggaran dicatat untuk ${selectedStudent.name} (+${selectedViolation.points} Poin). Net Poin: ${newNetPoints}`, 'success');
      } else {
        showToast(res.message || 'Gagal menyimpan transaksi pelanggaran.', 'error');
      }
    } catch (err: any) {
  ...
  const handleSaveEdit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setEditingTrx(null);
        await fetchTransactionsData();
        showToast(res.message || 'Data transaksi pelanggaran berhasil diperbarui.', 'success');
      } else {
  ...
  const handleDeleteTrx = (trx: Transaction) => {
    if (!isAdmin || isSaving) return; // Only admin can delete

    setAlertState({
      isOpen: true,
      type: 'warning',
      title: 'Hapus Transaksi Pelanggaran?',
      message: `Apakah Anda yakin ingin menghapus data pelanggaran "${trx.itemName}" atas nama ${trx.studentName}? Point siswa akan otomatis dikurangi kembali dari Google Spreadsheet.`,
      showCancel: true,
      onConfirm: async () => {
        showLoading('Menghapus transaksi pelanggaran...');
        try {
          ...
          if (res.success) {
            ...
            await fetchTransactionsData();
            showToast(res.message || 'Data transaksi pelanggaran berhasil dihapus.', 'success');
          } else {
          ...
          <div className="space-y-4 pt-2 border-t border-slate-100 dark:border-slate-700">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-slate-800 dark:text-slate-200 font-extrabold text-xs uppercase tracking-wider">
                ...
                <span>Detail Pelanggaran Tata Tertib (Terintegrasi Master Pelanggaran)</span>
              </div>
              <span className="text-[10px] font-bold text-slate-500 bg-slate-100 dark:bg-slate-700 px-2.5 py-1 rounded-full flex items-center gap-1">
                <Shield className="w-3.5 h-3.5 text-emerald-500" /> Poin Otomatis Readonly
              </span>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block font-bold text-slate-700 dark:text-slate-300 mb-1 flex items-center justify-between">
                  <span>Pilih Jenis Pelanggaran Dari Master Pelanggaran *</span>
                  {isViolationsLoading && (
                    <span className="text-xs text-blue-600 font-normal flex items-center gap-1">
                      <RefreshCw className="w-3 h-3 animate-spin" /> Memuat Master...
                    </span>
                  )}
                </label>
                
                {masterViolations.length === 0 && !isViolationsLoading ? (
                  <div className="p-3 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-xl text-amber-800 dark:text-amber-200 text-xs flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <AlertTriangle className="w-4 h-4 shrink-0" />
                      <span>Belum ada data Master Pelanggaran aktif.</span>
                    </div>
                    <button
                      type="button"
                      onClick={fetchMasterViolationsData}
                  ...
                  <select
                    ...
                    className="w-full p-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-bold dark:text-white cursor-pointer focus:ring-2 focus:ring-red-500"
                  >
                    <option value="">-- Pilih Jenis Pelanggaran (Diambil dari Master Pelanggaran) --</option>
                    {masterViolations.map(v => (
                      <option key={v.id} value={v.id}>
                        [{v.code || v.id}] [{v.category}] {v.name} (+{v.points} Poin)
                      </option>
                    ))}
                  </select>
                )}
              ...
              {/* AUTO-FILLED READ-ONLY DETAILS DISPLAY */}
              {selectedViolation && (
                <div className="p-4 bg-slate-50 dark:bg-slate-900/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-3 animate-in fade-in duration-150">
                  <div className="flex items-center justify-between border-b border-slate-200/80 dark:border-slate-700/80 pb-2">
                    <span className="font-extrabold text-slate-800 dark:text-slate-200 text-xs flex items-center gap-1.5">
                      ...
                      <span>Informasi Detail Master Pelanggaran (Readonly)</span>
                    </span>
                    <span className="text-[10px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/80 px-2 py-0.5 rounded-md border border-emerald-200 dark:border-emerald-800 flex items-center gap-1">
                      <CheckCircle2 className="w-3 h-3 text-emerald-500" /> Terverifikasi Master
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    ...
                    <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <div className="text-[10px] text-slate-400 font-bold uppercase">Kode Pelanggaran</div>
                      <div className="font-mono font-extrabold text-blue-600 dark:text-blue-400 text-sm mt-0.5">
                        {selectedViolation.code || selectedViolation.id}
                      </div>
                    </div>
                    ...
                    {/* Tindakan Standard */}
                    <div className="p-2.5 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700">
                      <div className="text-[10px] text-slate-400 font-bold uppercase">Rencana Tindakan</div>
                      <div className="font-bold text-slate-800 dark:text-slate-200 text-xs mt-0.5 truncate">
                        {selectedViolation.action || 'Teguran Lisan'}
                      </div>
                    </div>

                    {/* Poin Pelanggaran (Read-only) */}
                    <div className="p-2.5 bg-red-50 dark:bg-red-950/60 rounded-xl border border-red-200 dark:border-red-800/80 flex flex-col justify-between">
                      <div className="text-[10px] text-red-600 dark:text-red-400 font-bold uppercase flex items-center justify-between">
                        <span>Poin Pelanggaran</span>
                        <Shield className="w-3 h-3 text-red-500" />
                      </div>
                      <div className="font-black text-red-600 dark:text-red-400 text-base mt-0.5">
                        +{selectedViolation.points} Poin
                      </div>
                    </div>
                  </div>
                </div>
