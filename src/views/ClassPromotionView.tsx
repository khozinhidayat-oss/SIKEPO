...
export const ClassPromotionView: React.FC<ClassPromotionViewProps> = ({
  ...
  // State Management
  const { showLoading, hideLoading, showToast, updateProgress } = useLoading();
  const currentSettings = getSettings();
  const [oldAcademicYear, setOldAcademicYear] = useState<string>(currentSettings.academicYear || '2026/2027');
  const [semester, setSemester] = useState<string>(currentSettings.semester || 'Ganjil');
  ...
  // Single Source of Truth: Fetch Master Setting Sistem & preview data
  const loadData = async () => {
    try {
      let currYear = '2026/2027';
      let currSem = 'Ganjil';

      const settingsRes = await ApiService.getSettings();
      if (settingsRes.success && settingsRes.data) {
        currYear = settingsRes.data.academicYear || '2026/2027';
        currSem = settingsRes.data.semester || 'Ganjil';
      } else {
        const localSet = getSettings();
        currYear = localSet.academicYear || '2026/2027';
        currSem = localSet.semester || 'Ganjil';
      }
      ...
      // Get Promotion Preview from Master Setting Sistem
      const previewRes = await ApiService.getPromotionPreview(currYear, nextYear);
      const itemsData = previewRes.data || (previewRes as any).items;
      if (previewRes.success && Array.isArray(itemsData)) {
        setPreviewItems(itemsData);
      } else {
        setPreviewItems(getPromotionPreview(currYear, nextYear));
      }

      // Get Promotion History
      const histRes = await ApiService.getPromotionHistory();
      if (histRes.success && Array.isArray(histRes.data)) {
        setHistoryItems(histRes.data);
      } else {
        setHistoryItems(getPromotionHistory());
      }
    } catch (err) {
      const localSet = getSettings();
      const currYear = localSet.academicYear || '2026/2027';
      setOldAcademicYear(currYear);
      setSemester(localSet.semester || 'Ganjil');
      const nextYear = getNextAcademicYear(currYear);
      setNewAcademicYear(nextYear);
      setPreviewItems(getPromotionPreview(currYear, nextYear));
      setHistoryItems(getPromotionHistory());
  ...
  const handleRunSimulation = async () => {
    showLoading('Memuat simulasi kenaikan kelas...', 20);
    try {
      await loadData();
      showToast('Simulasi & preview kenaikan kelas berhasil diperbarui.', 'info');
    } catch (err: any) {
      showToast('Gagal memuat simulasi.', 'error');
    } finally {
      hideLoading();
    }
  };
  ...
  const handleExecutePromotion = () => {
    ...
    showAlert(
      ...
      async () => {
        ...
        showLoading('Memproses kenaikan kelas massal di Google Spreadsheet...', 10);

        try {
          updateProgress(50);
          const apiRes = await ApiService.executePromotion(previewItems, oldAcademicYear, newAcademicYear, userName);
          updateProgress(100);

          if (apiRes.success) {
            const resData = apiRes.data || {};
            await loadData();
            showToast(apiRes.message || `Kenaikan kelas selesai! ${resData.totalPromoted || 0} siswa naik kelas, ${resData.totalGraduated || 0} lulus.`, 'success');
          } else {
            showToast(apiRes.message || 'Gagal memproses kenaikan kelas di Google Apps Script.', 'error');
          }
        } catch (err: any) {
  ...
  const handleRollback = () => {
    ...
    showAlert(
      ...
      async () => {
        setIsProcessing(true);
        try {
          const apiRes = await ApiService.rollbackPromotion(userName, role);
          if (apiRes.success) {
            await loadData();
            showAlert('success', 'Rollback Berhasil!', apiRes.message || 'Proses kenaikan kelas berhasil dibatalkan.');
          } else {
            showAlert('error', 'Gagal Rollback', apiRes.message || 'Gagal melakukan rollback di Google Apps Script.');
          }
        } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 text-white p-6 rounded-2xl shadow-md border border-slate-700/50">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-4 border-b border-slate-700/60 gap-2">
          <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-emerald-400 bg-emerald-950/60 px-3 py-1.5 rounded-lg border border-emerald-800/60">
            <CheckCircle2 className="w-4 h-4 text-emerald-400 shrink-0" />
            <span>Master Setting Sistem (Single Source of Truth)</span>
          </div>
          <span className="text-xs text-slate-400 flex items-center gap-1">
            <Info className="w-3.5 h-3.5 text-blue-400 shrink-0" /> Sinkron dengan Google Spreadsheet DB
          </span>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          
          {/* Current Year (Master Setting) - Strictly Read Only */}
          <div className="md:col-span-5 bg-slate-800/90 p-4 rounded-xl border border-slate-700">
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-semibold uppercase text-slate-400 tracking-wider">
                Tahun Ajaran Aktif (Sistem)
              </span>
            ...
            <div className="flex items-center gap-3 mt-2">
              ...
              <div className="flex-1">
                <input
                  type="text"
                  value={oldAcademicYear || ''}
                  readOnly
                  disabled
                  className="w-full bg-slate-900/90 border border-slate-700 rounded-lg px-3 py-1.5 text-2xl font-black tracking-tight text-white cursor-not-allowed select-none focus:outline-none"
                />
                <p className="text-xs text-slate-400 mt-1 flex items-center gap-1">
                  Semester Aktif: <strong className="text-slate-200">{semester}</strong>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="md:col-span-2 flex flex-col items-center justify-center py-2 md:py-0">
            <div className="w-10 h-10 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center border border-emerald-500/30 shadow-inner">
              <ArrowRight className="w-5 h-5 animate-pulse" />
            </div>
            <span className="text-[11px] font-medium text-slate-400 mt-1 uppercase tracking-wider">Kenaikan</span>
          </div>

          {/* Target Year (Auto Computed) - Read Only */}
          <div className="md:col-span-5 bg-slate-800/90 p-4 rounded-xl border border-slate-700">
            <div className="flex items-center justify-between mb-1">
            ...
            <div className="flex items-center gap-2 mt-2">
              <input
                ...
                readOnly
                disabled
                className="w-full bg-slate-900/90 border border-emerald-800/80 rounded-lg px-3 py-1.5 text-2xl font-black text-emerald-300 cursor-not-allowed select-none focus:outline-none"
              />
              <button
                onClick={handleRunSimulation}
                disabled={isProcessing}
                className="shrink-0 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-semibold transition-colors flex items-center gap-1.5 shadow-sm disabled:opacity-50"
                title="Refresh Preview Simulasi"
              >
                <RefreshCw className="w-3.5 h-3.5" /> Reload
              </button>
            </div>
            <p className="text-[11px] text-slate-400 mt-2">
              Akan ditetapkan sebagai Tahun Ajaran Aktif baru di Master Setting Sistem setelah proses berhasil.
            </p>
          </div>
