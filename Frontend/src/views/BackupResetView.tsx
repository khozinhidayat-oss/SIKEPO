...
import { 
  getSpreadsheetConfig, saveSpreadsheetConfig, SpreadsheetConfig,
  getDriveFolderConfig, saveDriveFolderConfig, DriveFolderConfig,
  getBackupHistoryList, addBackupHistoryRecord, deleteBackupHistoryRecord, BackupRecord,
...
export const BackupResetView: React.FC<BackupResetViewProps> = ({
  ...
  // TAB 1: SPREADSHEET HANDLERS
  const handleTestSpreadsheet = () => {
    if (!ssConfig.spreadsheetId && !ssConfig.spreadsheetUrl) {
      showAlert('warning', 'Peringatan', 'Silakan masukkan Spreadsheet ID atau URL Spreadsheet terlebih dahulu.');
      return;
    }
    setIsTestingSs(true);
    setTimeout(() => {
      ...
      logActivity(userName, 'TEST_SPREADSHEET', `Uji koneksi Spreadsheet [${ssConfig.spreadsheetName}] berhasil`);
      showAlert('success', 'Koneksi Berhasil!', `Koneksi ke Google Spreadsheet "${ssConfig.spreadsheetName}" aktif dan valid (8 Sheet terverifikasi).`);
    }, 1200);
  };
  ...
  const handleDisconnectSpreadsheet = () => {
    showAlert(
      'warning',
      'Konfirmasi Putuskan Koneksi',
      'Apakah Anda yakin ingin memutuskan koneksi ke Google Spreadsheet ini?',
      () => {
        closeAlert();
        const updated = { ...ssConfig, status: 'Tidak Terhubung' as const };
        setSsConfig(updated);
        saveSpreadsheetConfig(updated);
        logActivity(userName, 'DISCONNECT_SPREADSHEET', 'Memutuskan koneksi Spreadsheet database');
        showAlert('info', 'Terputus', 'Koneksi Google Spreadsheet berhasil diputuskan.');
      },
      true,
      'Ya, Putuskan'
    );
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200 dark:border-slate-700/80 space-y-6">
          <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-slate-700">
            <div>
              <h2 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                ...
                Pengaturan Koneksi Google Spreadsheet
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                Hubungkan Spreadsheet aktif yang bertindak sebagai database Google Apps Script aplikasi SMART POINT SISWA.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Nama Spreadsheet</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">URL Google Spreadsheet</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={ssConfig.spreadsheetUrl}
                  ...
                  />
                  {ssConfig.spreadsheetUrl && (
                    <a
                      href={ssConfig.spreadsheetUrl}
                      target="_blank"
            ...
            <div className="bg-slate-50 dark:bg-slate-900/60 p-5 rounded-2xl border border-slate-200 dark:border-slate-700/60 space-y-4">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400">Ringkasan Info Database</h3>
              
              <div className="space-y-3 text-xs">
                <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-800">
                  <span className="text-slate-600 dark:text-slate-400">Status Database:</span>
                  <span className="font-bold text-slate-900 dark:text-white">{ssConfig.status}</span>
                </div>

                <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-800">
                  <span className="text-slate-600 dark:text-slate-400">Jumlah Sheet Terverifikasi:</span>
                  <span className="font-bold text-blue-600 dark:text-blue-400">{ssConfig.sheetCount} Sheet (users, students, violations, dll)</span>
                </div>

                <div className="flex justify-between items-center py-2 border-b border-slate-200 dark:border-slate-800">
                  <span className="text-slate-600 dark:text-slate-400">Terakhir Terhubung:</span>
                  <span className="font-semibold text-slate-800 dark:text-slate-200">{ssConfig.connectedAt || 'Belum terhubung'}</span>
                </div>

                <div className="p-3 bg-blue-50 dark:bg-blue-950/40 rounded-xl border border-blue-200 dark:border-blue-900/60 text-blue-800 dark:text-blue-300 text-[11px] leading-relaxed">
                  💡 <strong>Catatan:</strong> Memasukkan Spreadsheet ID otomatis akan mengonfigurasi `SpreadsheetApp.openById()` pada Google Apps Script Backend.
                </div>
              </div>
            </div>
          </div>
