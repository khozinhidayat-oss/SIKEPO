...
export const UsersView: React.FC<UsersViewProps> = ({ role, userName, onNavigateTab }) => {
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    const passwordHashValue = hashPassword(addForm.password);
    ...
    showAlert('success', 'User Berhasil Ditambahkan', `Akun untuk ${newUser.name} telah berhasil dibuat dan aktif.`);
  };
  ...
  const handleEditSubmit = (e: React.FormEvent) => {
    ...
    showAlert('success', 'Perubahan Disimpan', `Data pengguna ${editForm.name} berhasil diperbarui.`);
  };
  ...
  const handleResetPassSubmit = (e: React.FormEvent) => {
    ...
    showAlert(
      ...
      () => {
        const hashed = hashPassword(resetPassForm.newPassword);
        resetUserPassword(resetPassForm.userId, hashed);
        ...
        showAlert('success', 'Reset Password Berhasil', `Password untuk ${resetPassForm.userName} telah berhasil diperbarui.`);
      },
      true,
      'Ya, Reset Password'
    );
  ...
  const handleToggleStatus = (user: User) => {
    ...
    showAlert(
      ...
      () => {
        ...
        setUsersList(getUsers());
        showAlert('success', 'Status Berhasil Diubah', `Pengguna "${user.name}" sekarang berstatus ${nextStatus}.`);
      },
      true,
  ...
  const handleSoftDelete = (user: User) => {
    showAlert(
      ...
      () => {
        softDeleteUser(user.id, userName);
        logActivity(
          userName,
          role,
        ...
        setUsersList(getUsers());
        showAlert('success', 'Pengguna Dihapus', `Akun "${user.name}" berhasil ditandai sebagai Dihapus (Soft Delete).`);
      },
      true,
      'Ya, Hapus Pengguna'
    );
