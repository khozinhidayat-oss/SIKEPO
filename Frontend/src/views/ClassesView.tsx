...
export const ClassesView: React.FC<ClassesViewProps> = ({ role, userName }) => {
  ...
  const [formMajor, setFormMajor] = useState(majors[0]?.name || 'MIPA');
  ...
  const handleOpenAdd = () => {
    ...
    setFormName('');
    setFormLevel('X');
    setFormMajor(majors[0]?.name || 'MIPA');
  ...
  const handleOpenEdit = (item: ClassItem) => {
    ...
    setFormMajor(item.major || (majors[0]?.name || 'MIPA'));
  ...
  const handleSaveForm = (e: React.FormEvent) => {
    ...
    logActivity(
      userName, 
      role, 
      editingId ? 'EDIT_KELAS' : 'TAMBAH_KELAS', 
      `Kelas: ${formCode} - ${formName}`
    );
    ...
    setAlertState({
      ...
      type: 'success',
      title: 'Berhasil!',
      message: `Data kelas ${formName} (${formCode}) berhasil disimpan.`,
      onConfirm: () => setAlertState(prev => ({ ...prev, isOpen: false }))
    });
  };
  ...
  const handleDelete = (item: ClassItem) => {
    ...
    setAlertState({
      ...
      type: 'warning',
      title: 'Hapus Kelas?',
      message: `Apakah Anda yakin ingin menghapus kelas ${item.name} (${item.code})?`,
      showCancel: true,
      onConfirm: () => {
        deleteClass(item.id);
